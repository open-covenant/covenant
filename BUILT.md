...
- the Base trust layer beyond what is deployed: on-chain reputation writes (the EAS schema is registered, but no score is attested on-chain yet), funded USDC bonds or live slashing (the bond-receipt verifier is deployed but unfunded), a deployed Base x402 seller or Base-default outbound payments (both built, not wired), and trust-minimized cross-chain enforcement;
