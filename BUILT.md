...
- the Base trust layer beyond what is deployed: on-chain reputation writes (the EAS schema is registered, but no score is attested on-chain yet), funded USDC bonds or live slashing (the bond-receipt verifier is deployed but unfunded), Base-default outbound payments (built, not wired into the default daemon), and trust-minimized cross-chain enforcement;
