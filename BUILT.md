...
The agent-to-service payment rail over HTTP 402 (x402) is live today and is distinct from the internal-receipt settlement above: the daemon has a legacy-v1 outbound signer for metered resources, and Covenant operates a deployed x402-v2 seller that settles in USDC on Solana mainnet. Its paid endpoints return registration lookups, Covenant-authored signed statements, bounded PayAI-linked transfer observations, and enclave-verification results, with the seller's signing key published at `/.well-known/x402`. A valid signature authenticates Covenant as publisher; registration and payment observations do not prove real-world identity, x402 job delivery, quality, or reputation. Covenant also operates an escrow service and an Ephemeral-Rollup credit facilitator (the facilitator settles ER credits on devnet). What is *not* production is the daemon-driven anchoring of internal resource receipts to the settlement program; the payment rail being live does not change that boundary.

Covenant's signed-evidence projection also reaches Base mainnet (chain 8453) without moving the token: the foundation agent is registered under ERC-8004 (Identity Registry `0x8004A169FB4a3325136EB29fA0ceB6D2e539a432`, agent id 58403), a bond-receipt verifier (`0xBee387DD4A2fF215d6f997E5DA464C92285BCb6e`, `ecrecover`-based, native-USDC-denominated) and an EAS score schema are deployed, audit-root statements can be re-expressed as EAS off-chain statements signed by a cold secp256k1 issuer (`0x186953d5b4A290f8f53b8377cb38EDA75D664211`), and agents resolve through a live ENS CCIP-Read gateway (`opencovenant.eth`, `*.agents.opencovenant.eth` resolving to the Solana identity, served from `ens-gateway.opencovenant.org`). The crate also contains an experimental format utility for signing caller-supplied score projections, but it is not wired to publication and does not verify the supplied Solana reference or score derivation. `ecrecover` authenticates the publisher and signed bytes; it does not establish that an underlying claim is true. `$CVNT` stays a single Solana mint, every per-call fee and bond is chain-local USDC, and a build-time quarantine guard fails CI if the mint or a bridge primitive leaks into a cross-chain crate. A Base x402 seller is live as well, settling USDC on Base mainnet over EIP-3009 for Covenant-signed caller-supplied statements. What is *not* yet exercised on Base: no score has been written on-chain (the EAS schema is registered but writes stay gated), no USDC bond has been funded or slashed (the verifier is deployed but unfunded), the Base outbound-pay path is built but not wired into the default daemon, and trust-minimized cross-chain enforcement (slashing a Solana bond from an EVM-proven event) is unbuilt.
