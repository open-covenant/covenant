...
**Multi-chain reach.** Covenant also projects selected signed evidence onto Base mainnet — ERC-8004 agent registration, a deployed bond-receipt verifier, a registered EAS score schema, and an ENS CCIP-Read gateway (`*.agents.opencovenant.eth`) — while `$CVNT` stays a single Solana mint that is never bridged or wrapped and every per-call fee and bond is chain-local USDC. `ecrecover` authenticates the publisher and signed bytes; it does not establish that an underlying claim is true. On-chain score writes and funded USDC bonds are registered but not yet exercised, while live x402 sellers settle chain-local USDC on both Solana and Base mainnet. See [docs/multichain-value-capture.md](./docs/multichain-value-capture.md).
...
- Agent-to-service payments over HTTP 402 (x402): the daemon has a legacy-v1 outbound signer for metered resources, and Covenant operates a live x402-v2 seller that settles in USDC on Solana mainnet. Its paid endpoints return registration lookups, Covenant-authored signed statements, bounded PayAI-linked transfer observations, and enclave-verification results. A signature authenticates Covenant as publisher; registration and payment observations do not prove real-world identity, x402 job delivery, quality, or reputation.
- Multi-chain signed-evidence projection onto Base mainnet: ERC-8004 agent registration, a deployed bond-receipt verifier and EAS score schema, off-chain signed statements for audit roots and scores, and an ENS CCIP-Read gateway (`*.agents.opencovenant.eth`) that resolves to a configured Solana registration. An `ecrecover` authenticates the publisher and signed bytes, not the truth of a claim. On-chain score writes and funded USDC bonds are not yet exercised. See [docs/multichain-value-capture.md](./docs/multichain-value-capture.md).
