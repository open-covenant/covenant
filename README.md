...
Covenant sits below agent applications and above the host operating system. It owns the state, authority, and accountability concerns that recur across agent frameworks — scoped capabilities, durable memory, controlled runtime dispatch, local hash-chained audit, and commit-scoped provenance — so individual frameworks can stop reinventing them.

<!-- METRICS:START -->
**Status.** Local control plane is real and live-tested (39 Rust crates, ~262k lines, 3571 source-discovered Rust tests including 472 live boundary tests). Production-grade sandboxing for hostile agent code and networked multi-peer operation are roadmap; the Solana settlement program is deployed on mainnet (credits, staking, slashing, on-chain receipt anchoring), but its daemon-driven economic lifecycle is not yet production. See [BUILT.md](./BUILT.md) for the explicit honesty boundary.
<!-- METRICS:END -->

**Multi-chain reach.** Covenant also projects selected signed evidence onto Base mainnet — ERC-8004 agent registration, a deployed bond-receipt verifier, a registered EAS score schema, and an ENS CCIP-Read gateway (`*.agents.opencovenant.eth`) — while `$CVNT` stays a single Solana mint that is never bridged or wrapped. The Solana/Base evidence and bond surfaces described here use chain-local USDC; other integrations can use their own settlement assets. `ecrecover` proves that bytes were signed by the configured address; it does not establish the signer&rsquo;s real-world identity or that an underlying claim is true. On-chain score writes and funded USDC bonds are registered but not yet exercised, while live x402 sellers settle chain-local USDC on both Solana and Base mainnet. See [docs/multichain-value-capture.md](./docs/multichain-value-capture.md).
...
The console proxies the daemon's HTTP gateway, injects the operator bearer token server-side, and renders recorded Covenant dispatch events as a local hash-chain trace. That trace supports consistency checks; it does not prove that every host action passed through Covenant or that the writer recorded every event. See [examples/hello-agent](./examples/hello-agent/) for the agent walkthrough, [docs/demo.md](./docs/demo.md) for a CLI transcript, and [deploy/README.md](./deploy/README.md) for shipping the console as a public sandbox on Render.
...
- Agent-to-service payments over HTTP 402 (x402): the daemon contains a parked legacy-v1 outbound signer path, and Covenant operates a live x402-v2 seller that settles in USDC on Solana mainnet. Its paid endpoints return registration observations, publisher-signed caller-supplied statements, bounded PayAI-linked transfer observations, and seller-reported DCAP-monitor results. A signature proves only that the bytes were signed by the accompanying key; attributing that key to Covenant requires an expected key pinned through a trusted external channel. Registration and payment observations do not prove real-world identity, x402 job delivery, quality, or reputation.
- Multi-chain signed-evidence projection onto Base mainnet: ERC-8004 agent registration, a deployed bond-receipt verifier and EAS score schema, off-chain signed statements for audit roots, an experimental formatter for caller-supplied score projections that is not wired to publication, and an ENS CCIP-Read gateway (`*.agents.opencovenant.eth`) that resolves to a configured Solana registration. `ecrecover` proves that bytes were signed by the configured address; it does not establish signer identity, score derivation, the supplied Solana reference, or claim truth. On-chain score writes and funded USDC bonds are not yet exercised. See [docs/multichain-value-capture.md](./docs/multichain-value-capture.md).
