...
Covenant projects bounded signed statements and bond receipts onto other chains without moving `$CVNT` off Solana. Live today on Base mainnet: ERC-8004 agent registration, a deployed bond-receipt verifier, a registered EAS reputation schema, EAS off-chain audit-root statements, and an ENS CCIP-Read gateway (`opencovenant.eth`, `*.agents.opencovenant.eth`) that resolves a configured Solana identifier. A recovered signature proves that its configured key signed the exact bytes; attribution and claim truth remain external. A live x402 seller also accepts USDC payments on Base mainnet over EIP-3009. `$CVNT` stays a single Solana mint. Configured per-call charges and bond units use chain-local USDC; routing any resulting revenue into Solana buy-and-lock staking is not implemented, and a build-time quarantine guard prevents unsupported bridge paths from being presented as live.
...
- The agent-to-service payment rail over HTTP 402 (x402) is operating on the seller side: Covenant runs a live x402 seller that settles USDC on Solana mainnet, alongside an escrow service and an Ephemeral-Rollup credit facilitator. Lower-level outbound client and signer components remain available for explicit development use, but daemon-owned outbound payment is parked until exact one-use authorization and durable prepayment reservation are implemented. Internal receipt anchoring, outbound wallet enforcement, and crash-safe reconciliation therefore remain settlement work.
