                ...
                Ok(dir) => {
                    if let Err(e) = dir.sync_all().await {
                        tracing::warn!(
                            ...
                            error = %e,
                            "budget ledger: parent directory fsync failed; compact may not be crash-durable",
                        );
                    }
                }
                Err(e) => tracing::warn!(
                    path = %self.path.display(),
                    error = %e,
                    "budget ledger: could not open parent directory for fsync",
                ),
            }
        }
...
mod tests {
    ...
    #[test]
    fn budget_checkpoint_error_io_source_delegation_pin_returns_inner_std_io_error_via_std_error_source(
    ) {
        use std::error::Error;

        let inner = std::io::Error::new(
            std::io::ErrorKind::NotFound,
            "checkpoints.jsonl: missing on resume",
        );
        let expected_display = format!("{inner}");
        let err = BudgetCheckpointError::Io(inner);
        let source = err.source().expect(
            "covenant_budget::BudgetCheckpointError::Io must surface the inner std::io::Error via std::error::Error::source so daemon-side pause-checkpoint retry classifiers can walk the error chain and downcast source() to std::io::Error to extract io::ErrorKind for distinct decisions on pause-checkpoints.jsonl IO (NotFound means no pause checkpoint exists yet — the resume verb returns AlreadyResumed-equivalent semantics, PermissionDenied escalates to operator-attention on a file present but unreadable, Interrupted retries the read immediately, NoSpace blocks new pause records); a refactor that converted the variant from #[from] to a hand-written Error impl returning None (under a 'simpler error wrapping' rationale) would silently change source() to return None while leaving Display intact (dropped-source-attribute regression class)",
        );
        assert_eq!(
            format!("{source}"),
            expected_display,
            "covenant_budget::BudgetCheckpointError::Io source() Display must match a direct format!() of the same std::io::Error verbatim; a refactor that swapped the inner field type to Box<dyn Error + Send + Sync> or any other wrapper would silently break daemon-side downcasts even though the wrapper's Display would continue to flow through {{0}} (concrete-source-type regression class)"
        );
        let kind = source.downcast_ref::<std::io::Error>().map(|e| e.kind());
        assert_eq!(
            kind,
            Some(std::io::ErrorKind::NotFound),
            "covenant_budget::BudgetCheckpointError::Io source() must downcast_ref to std::io::Error so daemon-side pause-checkpoint retry classifiers can extract io::ErrorKind for distinct decisions on pause-checkpoints.jsonl IO; a refactor that wrapped the inner in a project-local newtype (e.g., BudgetCheckpointIoError(std::io::Error) under a 'tag pause-checkpoint IO failures distinctly from sibling Io variants in other crates' rationale) would silently break downcast_ref::<std::io::Error>() at every downstream callsite that classifies pause-checkpoint IO faults (concrete-source-type downcast regression class)"
        );
    }

    #[test]
    fn budget_checkpoint_error_serde_source_delegation_pin_returns_inner_serde_json_error_via_std_error_source(
    ) {
        use std::error::Error;
        ...
        let source = err.source().expect(
            "covenant_budget::BudgetCheckpointError::Serde must surface the inner serde_json::Error via std::error::Error::source so daemon-side pause-checkpoint diagnostics can walk the error chain and downcast source() to serde_json::Error to inspect line/column for malformed-checkpoint identification (line/column points the operator at the offending checkpoints.jsonl row, classify() distinguishes Syntax-vs-Data-vs-EOF for incident triage); a refactor that converted the variant from #[from] to a hand-written Error impl returning None (under a 'simpler error wrapping' rationale) would silently change source() to return None while leaving Display intact (dropped-source-attribute regression class)",
        );
        assert_eq!(
            format!("{source}"),
            expected_display,
            "covenant_budget::BudgetCheckpointError::Serde source() Display must match a direct format!() of the same serde_json::Error verbatim; a refactor that swapped the inner field type to Box<dyn Error + Send + Sync> or any other wrapper would silently break daemon-side downcasts even though the wrapper's Display would continue to flow through {{0}} (concrete-source-type regression class)"
        );
        assert!(
            source.downcast_ref::<serde_json::Error>().is_some(),
            "covenant_budget::BudgetCheckpointError::Serde source() must downcast_ref to serde_json::Error so daemon-side pause-checkpoint diagnostics can call serde_json::Error::line/column/classify for malformed-checkpoint identification; a refactor that wrapped the inner in a project-local newtype (e.g., BudgetCheckpointSerdeError(serde_json::Error) under a 'consolidate parse errors into one Wire variant' rationale) would silently break downcast_ref::<serde_json::Error>() at every downstream callsite that classifies pause-checkpoint row parse faults (concrete-source-type downcast regression class)"
        );
    }
