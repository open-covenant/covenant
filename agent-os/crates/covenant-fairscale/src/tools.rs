...
//! labeled soft signal, with a [`crate::provenance::ReadProvenance`] block the
//! daemon anchors so the read is verifiable. It never moves funds. The result
...
impl Tool for ScoreTool {
    ...
    async fn call(&self, arguments: Value) -> Result<ToolCallResult, ToolError> {
        ...
        if self.include_trust_gate {
            if let Ok(gate) = self.client.trust_gate(pubkey, 60).await {
        ...
        if self.credit_enabled {
            if let Ok(credit) = self.client.credit(pubkey, 1000).await {
                summary["credit"] = json!({
