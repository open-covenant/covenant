//! Native Rust tools for the v0 registry.
...
mod tests {
    ...
    #[test]
    fn echo_input_schema() {
        let schema = EchoTool.input_schema();
        let obj = schema
            .as_object()
            .expect("input_schema emits a JSON object");
        ...
        let text_spec = properties
            .get("text")
            .and_then(|v| v.as_object())
            .expect("properties.text is an object");
        assert_eq!(
            text_spec.get("type").and_then(|v| v.as_str()),
            Some("string"),
            "properties.text.type is string, matching call()'s as_str read",
        );

        let required = obj
            .get("required")
            .and_then(|v| v.as_array())
            .expect("required is a JSON array");
        assert_eq!(required.len(), 1, "exactly one required field");
        assert_eq!(
            required[0].as_str(),
            Some("text"),
            "the required field is 'text'"
        );
        ...
        assert_eq!(
            additional,
            &Value::Bool(false),
            "additionalProperties is the bool false (closed-world)",
        );
    ...
    #[test]
    fn echo_and_clock_name_and_description() {
        assert_eq!(
            EchoTool.name(),
            "echo",
            "EchoTool name is the selector/sort key"
        );
        assert_eq!(
            EchoTool.description(),
            "Returns the provided `text` argument verbatim.",
            "EchoTool description names the `text` argument operators pass",
        );
        assert_eq!(
            ClockTool.name(),
            "clock",
            "ClockTool name is the selector/sort key"
        );
        assert_eq!(
            ClockTool.description(),
            "Returns the current Unix time in milliseconds.",
            "ClockTool description keeps the load-bearing 'milliseconds' unit",
        );
    }
