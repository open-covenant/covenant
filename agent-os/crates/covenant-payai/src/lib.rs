...
//! money path. This crate reads PayAI's public on-chain settlements and turns
//! them into two trust primitives:
//!
//! - **Phase 0** — settlement-grounded reputation per agent wallet, signed by
//!   the Covenant identity so it is a verifiable credential, not a webpage.
//! - **Phase 1** — Covenant-signed work-receipts binding a settlement to the
//!   work delivered, with an optional buyer counter-signature.
//!
//! It does NOT move funds, hold escrow, or call verify/settle. Everything here
//! is read-after-settlement + attest. Solana is read over plain JSON-RPC via
//! `reqwest`, so the crate stays light and daemon-linkable (no `solana-sdk`).
...
/// PayAI's Solana facilitator fee-payer. It co-signs/pays for every PayAI
/// Solana settlement, so watching it yields the complete settlement stream.
...
/// True for either USDC mint Covenant recognises.
