...
//!
//! Confirms `ipc::recent_audit` returns `IpcError::DaemonNotRunning`
...
//! `covenantd start` hint. `recent_audit` is used because it has no
//! capability gate — the failure is unambiguously about the missing
//! socket, not about an unrelated read-side check.
//!
...
use covenant_tui::ipc::{recent_audit, IpcError};
