...
//! forge the verdict.
//!
//! The attestor key is distinct from the ed25519 trade-receipt attestor in
//! [`crate::robinhood`]: the escrow verifies on an EVM chain, so the verdict is
//! signed over secp256k1, and the key's [`address`](SpendGrantAttestor::address)
