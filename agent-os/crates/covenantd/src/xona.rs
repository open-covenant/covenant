//! Daemon glue for the Xona Agent x402 provider profile.
//!
//! `covenant-xona` owns the catalog and the MCP tool surface but knows
//! nothing about the daemon's funding key, budget, or settlement logs —
//! it reaches them through a [`covenant_xona::PaidExecutor`]. This module
//! is that implementation: [`DaemonXonaExecutor`] binds one caller (the
//! payer) to the daemon's accounting subsystems and routes a resolved
//! Xona call through the same [`crate::x402::record_paid_call`] path
//! every other outbound x402 payment uses. A Xona receipt is a plain
//! `ResourceKind::Tool` receipt — it rolls into the same Merkle batch and
//! optional Synapse mirror with no Xona-specific surface.
//!
...
//! Xona's Solana endpoints are self-paid (no sponsor `feePayer`), so the
//! sidecar settles them with `SolanaSigner`.
...
use covenant_xona::{PaidExecutor, PaidRequest, PaidResponse, XonaCatalog, XonaConfig};
use serde_json::Value;
use tracing::warn;

use crate::x402::{record_paid_call, PaidCall, SettlementContext, X402Config};

/// Materialised Xona catalog plus its config, built once at daemon
/// startup and shared behind an `Arc`. Rebuilt out of band when the
/// daemon refreshes the registry.
pub struct XonaState {
    pub catalog: XonaCatalog,
    pub config: XonaConfig,
}

impl XonaState {
    pub fn new(catalog: XonaCatalog, config: XonaConfig) -> Self {
        Self { catalog, config }
    }
}
...
#[async_trait]
impl PaidExecutor for DaemonXonaExecutor {
    async fn execute(&self, req: PaidRequest) -> Result<PaidResponse, String> {
        ...
        let out = covenant_xona::execute_paid(&http, &signer, &req)
            .await
            ...
            Some(amount) if (200..300).contains(&out.status) => {
                ...
                match record_paid_call(&ctx, &self.payer, &call).await {
                    ...
                    Err(e) => {
                        ...
                        warn!(error = %e, endpoint = %req.url, "xona paid call succeeded but accounting failed");
...
mod tests {
    ...
    fn req() -> PaidRequest {
        PaidRequest {
            ...
            url: "https://api.xona-agent.com/image/creative-director".into(),
            method: "POST".into(),
            body: Some(serde_json::json!({ "prompt": "x" })),
            network: covenant_xona::config::SOLANA_NETWORK.into(),
            asset: covenant_xona::config::USDC_MINT.into(),
            per_call_cap: 30_000,
            credits: 3,
            price_micro_usdc: 30_000,
            pay_to: covenant_xona::config::PAY_TO.into(),
        }
    }
    ...
    /// With the funding sidecar disabled the executor must refuse before
    /// any network or signer activity and write no accounting.
    #[tokio::test]
    async fn executor_refuses_when_x402_disabled() {
        ...
        let exec = DaemonXonaExecutor::new(
            settlement.clone(),
            audit.clone(),
            budget.clone(),
