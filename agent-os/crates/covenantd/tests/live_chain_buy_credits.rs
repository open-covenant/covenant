//! Live coverage for `covenant chain buy-credits --json`.
//!
//! End-to-end path: spawns a real covenantd against a temp HOME so
//! the CLI passes peer-auth, then runs the `buy-credits` verb
//! against a real Solana validator with the settlement program
//! deployed. The verb signs locally with the operator keypair and
//! submits + confirms via the Solana RPC URL.
//!
...
//!     `COVENANT_LIVE_CHAIN_OWNER_COVNT`, and the treasury token
...
#[tokio::test]
#[ignore = "live: spawns covenantd + submits `covenant chain buy-credits --json` against a localnet validator with a deployed settlement program + initialized credits PDA + funded owner_covnt + treasury that matches config.treasury"]
async fn live_cli_chain_buy_credits_submits_and_confirms() {
    ...
    let treasury = match env_or_skip("COVENANT_LIVE_CHAIN_TREASURY") {
        Some(v) => v,
        None => {
            eprintln!(
                "skip: COVENANT_LIVE_CHAIN_TREASURY not set. \
                 Point it at the config.treasury value (fetch via `chain status`)."
            );
            return;
        }
    };
    let amount = std::env::var("COVENANT_LIVE_CHAIN_BUY_CREDITS_AMOUNT")
    ...
    let output = Command::new(covenant_cli_bin())
        ...
        .output()
        .await
        .expect("run covenant chain buy-credits");
    ...
    assert!(
        ...
        "buy-credits failed: status={:?} stdout={stdout} stderr={stderr}",
        output.status,
    );
    ...
    assert_eq!(envelope["verb"], "buy-credits");
    ...
    assert!(
        envelope["amount_covnt"].is_number(),
        "amount_covnt is numeric"
    );
    let owner = envelope["owner"]
        .as_str()
        .expect("owner is the operator pubkey as a base58 string");
    assert!(!owner.is_empty(), "owner is non-empty base58");
}
