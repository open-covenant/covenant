//! Live CLI coverage for `covenant a2a compact --json`.
...
#[tokio::test]
#[ignore = "live: spawns covenantd + runs `covenant a2a compact --json` subprocess"]
async fn live_cli_a2a_compact_json_round_trip() {
    ...
    run_cli(
        &cli_exe,
        home.path(),
        &["capabilities", "grant", "a2a.compact"],
    )
    .await;
    let stdout = run_cli(&cli_exe, home.path(), &["a2a", "compact", "--json"]).await;

    let value: Value =
        serde_json::from_str(stdout.trim()).expect("a2a compact --json must be valid JSON");
    assert_eq!(value["kind"].as_str(), Some("a2a_compacted"));
    assert_eq!(value["dropped"].as_u64(), Some(0));
