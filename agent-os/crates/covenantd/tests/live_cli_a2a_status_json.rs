//! Live CLI coverage for `covenant a2a status --json`.
//!
...
#[tokio::test]
#[ignore = "live: spawns covenantd + runs `covenant a2a status --json` subprocess"]
async fn live_cli_a2a_status_json_round_trip() {
    ...
    assert!(
        ...
        "a2a status --json must not emit stderr on success: {stderr:?}"
    );

    let value: Value =
        serde_json::from_str(stdout.trim()).expect("a2a status --json must be valid JSON");
    assert_eq!(value["kind"].as_str(), Some("a2a_status"));
    assert_eq!(value["limit"].as_u64(), Some(5));
    assert_eq!(value["min_lease_age_ms"].as_u64(), Some(300_000));
    assert!(value["tasks"].as_array().expect("tasks array").is_empty());
