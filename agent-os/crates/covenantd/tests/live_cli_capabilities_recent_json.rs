//! Live CLI coverage for `covenant capabilities recent --json`.
//!
//! Spawns a real daemon against a temp home, grants one capability
//! through the CLI, then asserts stdout is one stable JSON object.
...
#[tokio::test]
#[ignore = "live: spawns covenantd + runs `covenant capabilities recent --json` subprocess"]
async fn live_cli_capabilities_recent_json_round_trip() {
    ...
    let stdout = run_cli(
        &cli_exe,
        home.path(),
        &["capabilities", "recent", "--json", "--limit", "10"],
    )
    .await;

    let value: Value =
        serde_json::from_str(stdout.trim()).expect("capabilities recent --json must be valid JSON");
    ...
    assert_eq!(caps.len(), 1, "expected one capability: {value:?}");
    assert_eq!(
        caps[0]["capability"]["action"].as_str(),
        Some("tool.call.echo")
    );
    assert!(
        caps[0]["signature"]
        ...
        "signature must be a base58 string: {value:?}"
    );
