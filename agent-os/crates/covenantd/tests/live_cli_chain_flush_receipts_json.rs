//! Live CLI coverage for `covenant chain flush-receipts --json`.
//!
//! Spawns a real daemon against a temp home, grants the capabilities
//! needed to write and flush local receipts, submits one intent through
//! the CLI, then asserts the flush command emits one stable JSON object.
//! Opt-in because it crosses process and socket boundaries. Run from
//! `agent-os/` after `cargo build -p covenant`.
...
#[tokio::test]
#[ignore = "live: spawns covenantd + runs `covenant chain flush-receipts --json` subprocess"]
async fn live_cli_chain_flush_receipts_json_round_trip() {
    ...
    run_cli(
        &cli_exe,
        home.path(),
        &["intent", "flush receipts json probe"],
    )
    .await;
    ...
    let stdout = run_cli(
        &cli_exe,
        home.path(),
        &["chain", "flush-receipts", "--json", "--limit", "10"],
    )
    .await;
    let value: Value =
        serde_json::from_str(stdout.trim()).expect("flush-receipts --json must be valid JSON");

    assert_eq!(value["kind"].as_str(), Some("receipt_batch_flushed"));
    assert_eq!(value["limit"].as_u64(), Some(10));
    assert_eq!(value["receipts_updated"].as_u64(), Some(1));
    assert_eq!(value["batch"]["receipt_count"].as_u64(), Some(1));
    assert!(
        value["batch"]["batch_id"]
            .as_str()
            .is_some_and(|s| !s.is_empty()),
        "batch must include id: {value:?}"
    );
    assert!(
        value["batch"]["merkle_root"]
            .as_str()
            .is_some_and(|s| !s.is_empty()),
        "batch must include merkle root: {value:?}"
    );
