//! Live CLI coverage for `covenant peers purge --json`.
...
#[tokio::test]
#[ignore = "live: spawns covenantd + runs `covenant peers purge --json` subprocess"]
async fn live_cli_peers_purge_json_round_trip() {
    ...
    let registry_path = home.path().join("peers").join("registry.jsonl");
    {
        ...
        registry
            .register(PeerEntry {
                token: guest_token,
                agent_id: AgentId::new("guest-purge-json@local", [42u8; 32]),
    ...
    run_cli(
        &cli_exe,
        home.path(),
        &["capabilities", "grant", "peers.purge"],
    )
    .await;
    ...
    let stdout = run_cli(
        &cli_exe,
        home.path(),
        &["peers", "purge", "--before-ms", before_ms, "--json"],
    )
    .await;

    let value: Value =
        serde_json::from_str(stdout.trim()).expect("peers purge --json must be valid JSON");
