//! Live CLI coverage for `covenant peers rotate --json`.
...
#[tokio::test]
#[ignore = "live: spawns covenantd + runs `covenant peers rotate --json` subprocess"]
async fn live_cli_peers_rotate_json_round_trip() {
    ...
    let old_token = read_operator_token(home.path()).await;
    let cli_exe = covenant_cli_bin();
    let stdout = run_cli(&cli_exe, home.path(), &["peers", "rotate", "--json"]).await;

    let value: Value =
        serde_json::from_str(stdout.trim()).expect("peers rotate --json must be valid JSON");
    assert_eq!(value["kind"].as_str(), Some("peer_token_rotated"));
    let new_token = value["token_b58"]
        .as_str()
        .expect("token_b58 must be present");
    assert!(!new_token.is_empty(), "new token must not be empty");
    assert_ne!(new_token, old_token, "rotation must mint a new token");

    let on_disk = read_operator_token(home.path()).await;
    assert_eq!(on_disk, new_token, "operator.token must hold rotated token");

    run_cli(&cli_exe, home.path(), &["ping"]).await;
