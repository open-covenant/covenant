//! Live CLI coverage for `covenant ping --json`.
...
#[tokio::test]
#[ignore = "live: spawns covenantd + runs `covenant ping --json` subprocess"]
async fn live_cli_ping_json_round_trip() {
    ...
    let stdout = run_cli(&cli_exe, home.path(), &["ping", "--json"]).await;
    let value: Value = serde_json::from_str(stdout.trim()).expect("ping --json must be JSON");
