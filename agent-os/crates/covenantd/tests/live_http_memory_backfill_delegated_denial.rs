...
//! `live_ipc_memory_backfill_delegated_denial.rs`; this extends the same pin
//! to HTTP.
//!
...
//! bearer-authed delegate rewrite the operator's memory-record provenance
//! over the network. Self-granting first is what makes this an
//! operator-identity pin and not a capability pin: the delegate clears the
//! capability gate, so the refusal must name the identity layer. A default
//! dry_run reaches the success response once the gate is off, so the
//! never-MemoryRecordsBackfilled guard is not vacuous.
//!
//! Hermetic — no external services. `#[ignore]`'d. Run with
//! `cargo test -p covenantd --test live_http_memory_backfill_delegated_denial -- --ignored live_`.
...
#[tokio::test]
#[ignore = "live: spawns covenantd + verifies a self-granted bearer delegate cannot backfill memory records over HTTP"]
async fn live_http_memory_backfill_rejects_self_granted_non_operator() {
    let home = tempfile::tempdir().expect("tempdir");

    let delegate_token = PeerToken::from_bytes([175u8; 32]);
    let delegate_token_b58 = delegate_token.to_b58();
    let delegate_pubkey = [176u8; 32];
    let delegate_display = "delegate-http-memory-backfiller@local";
    ...
    // ── Phase 1: the delegate self-grants memory.backfill.dry_run over the
    //     gateway. The capability subject is the authenticated bearer peer,
    //     so the grant lands and clears the capability gate for the backfill
    //     request. No scope is sent, so the grant stores the empty `{}` scope
    //     that is unbounded and clears the before_ms = u64::MAX scope probe.
    let grant_response = client
        .post(format!("{base}/capabilities/grant"))
        .json(&json!({ "action": "memory.backfill.dry_run" }))
        .send()
        .await
    ...
    assert_eq!(
        ...
        "delegate self-grant of memory.backfill.dry_run must succeed; got {grant_json:?}",
    );
    ...
    //     (which still contains "memory.backfill"), and NOT the
    //     memory_records_backfilled kind that the same request returns once
    //     the gate is dropped. That distinction is what catches a handler
    //     forwarding the operator identity instead of the bearer delegate.
    let backfill_response = client
        .post(format!("{base}/memory/records/backfill"))
        .json(&json!({ "dry_run": true, "scope_pubkey": null }))
        .send()
        .await
        .expect("send /memory/records/backfill request");
    let backfill_json: Value = backfill_response
        .json()
        .await
        .expect("/memory/records/backfill response body");
    assert_eq!(
        backfill_json["kind"], "error",
        "a self-granted delegate must be refused the backfill, never memory_records_backfilled; \
         got {backfill_json:?}",
    );
    ...
    assert!(
        ...
        "HTTP /memory/records/backfill must refuse the delegate at the operator-identity gate \
         that fires after the capability gate; got {message:?}",
    );
    assert!(
