...
//! delegate which SELF-GRANTS `settlement.backfill.dry_run` is still
...
//! "settlement backfill requires the operator identity"` with no
...
//! capability. `grant_capability` (lib.rs:6076) stores a `None` scope as
...
//! post-gate scope probe (which checks `before_ms = u64::MAX`), leaving
//! the operator-identity gate as the sole remaining barrier between the
//! delegate and a backfill rewrite over the operator's receipt store.
//!
...
//! `Response::SettlementReceiptsBackfilled`, so the gate is load-bearing.
//!
//! Hermetic — no external services. `#[ignore]`'d. Run with
//! `cargo test -p covenantd --test live_ipc_settlement_backfill_delegated_denial -- --ignored live_`.
...
#[tokio::test]
#[ignore = "live: spawns covenantd + verifies a self-granted delegate cannot backfill settlement receipts"]
async fn live_covenantd_settlement_backfill_rejects_self_granted_non_operator() {
    ...
    let delegate_display = "delegate-settlement-backfiller@local";
    ...
    // ── Phase 1: the delegate self-grants settlement.backfill.dry_run. The
    ...
    //     succeeds and clears the capability gate for the next request.
    {
        ...
        .await
        {
            ...
            other => panic!(
                "delegate self-grant of settlement.backfill.dry_run must succeed, got {other:?}"
            ),
        }
    }

    // ── Phase 2: holding the capability, the delegate sends a dry-run
    //     BackfillSettlementReceipts with no scope_pubkey. It is past the
    ...
    //     Response::SettlementReceiptsBackfilled (which is exactly what the
    //     same request returns once the gate is removed).
    {
        let mut stream = authenticated_stream(&sock, &delegate_token_b58).await;
        match req(
            &mut stream,
        ...
        .await
        {
            ...
            other => panic!(
                "a delegate holding settlement.backfill.dry_run must still be refused, never \
                 Response::SettlementReceiptsBackfilled: got {other:?}"
            ),
        }
    }
