...
// inside capabilities_purge_json_pins_top_level_schema:
//
//   - line 283 cites `before_ms` (u64) type pin.
//   - line 284 cites `purged` (u64) type pin.
//
// Both cites are correct under the assert!-opener-to-closer
// 4-line convention. The validator scopes each lookup to the
// brace-balanced capabilities_purge_json_pins_top_level_schema fn
// body so sibling pins-tests that share the same value["before_ms"]
// .is_u64() and value["purged"].is_u64() selectors (peers_purge,
// audit_purge, memory_purge) cannot contaminate the result. Pattern
// mirrors validate-audit-purge-type-level-pin-line-refs.mjs.
...
const targets = [
  {
    ...
    docsLabel: "capabilities_purge.before_ms type-level pin citation",
    docsTemplate:
  ...
  },
  {
    ...
    docsRegex:
      /- `purged` \(u64\): the count of revoked-capability rows removed\. May legitimately be `0` when no rows matched the cutoff — the verb does not error on an empty purge\. Pinned as u64 by `main\.rs:(\d+)-(\d+)` — never a string-of-integer\./,
    docsLabel: "capabilities_purge.purged type-level pin citation",
    docsTemplate:
      "Pinned as u64 by `main.rs:N-M` — never a string-of-integer.",
  },
];
...
console.log(
  `validate-capabilities-purge-type-level-pin-line-refs: ok (${targets.map((t) => `${t.field} main.rs:${t.startLine}-${t.endLine}`).join(", ")})`,
);
