...
if (source) {
  ...
  if (testMatches.length !== 1) {
    fail(
      `${sourcePath}: expected exactly 1 "fn ${testFnName}" inside tests module but found ${testMatches.length}; remediation: confirm the capability_revoke pins-test still exists and is not renamed or duplicated`,
    );
  } else {
...
if (errors.length > 0) {
  console.error("validate-capability-revoke-type-level-pin-line-refs: failed");
