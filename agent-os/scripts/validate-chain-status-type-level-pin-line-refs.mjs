...
// chain_status envelope type-level pin line-ref drift guard.
// docs/ipc-and-http-gateway.md line 98 cites a single main.rs range
// for the chain_status.status (object) type pin inside the
// chain_status_json_pins_top_level_schema fn body.
//
// Before this validator landed, the docs prose said "the inner
// `status` is never a string blob" with no main.rs cite; the
// assertion at main.rs:7154-7157 (assert!(value["status"].is_object(),
...
// field into a non-object type surfaces at the docs-validator level
// (not just at test runtime).
//
// The validator scopes its lookup to the brace-balanced
// `chain_status_json_pins_top_level_schema` fn body. The
// value["status"].is_object(), selector is currently unique inside
// main.rs (this is the only chain_status envelope assertion using
// that exact form), but the brace-scoping plus exact-trim-match
// also isolates the chain_status one if a sibling envelope later
...
// validate-audit-verify-type-level-pin-line-refs.mjs.
...
const selector = 'value["status"].is_object(),';

const docsRegex =
  /- `status`: a structured `covenant_ipc::ChainStatus` object with the following fields\. The top-level object has exactly two keys \(`kind` and `status`\); the inner `status` is pinned by the schema test at `main\.rs:(\d+)-(\d+)` to be a JSON object, never a string blob\./;
const docsLabel = "chain_status.status type-level pin citation";
const docsTemplate =
  "the inner `status` is pinned by the schema test at `main.rs:N-M` to be a JSON object, never a string blob.";
...
if (source) {
  ...
  if (testMatches.length !== 1) {
    fail(
      `${sourcePath}: expected exactly 1 "fn ${testFnName}" inside tests module but found ${testMatches.length}; remediation: confirm the chain_status pins-test still exists and is not renamed or duplicated`,
    );
  } else {
...
if (errors.length > 0) {
  console.error("validate-chain-status-type-level-pin-line-refs: failed");
...
console.log(
  `validate-chain-status-type-level-pin-line-refs: ok (chain_status.status main.rs:${startLine}-${endLine})`,
);
