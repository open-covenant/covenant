...
// daemon_ping kind literal value pin line-ref drift guard.
// docs/ipc-and-http-gateway.md line 225 cites a main.rs line for the
// value-level pin on
// `value["kind"].as_str() == Some("daemon_ping")` inside
// `ping_json_pins_top_level_schema`. The sibling is_string type pin
// at main.rs:5852 is the type-level guard already implicit in the
// pins-test fn line ref; this validator binds the docs prose to the
// discriminator-value assertion so a future kind-rename (for
// example, splitting daemon_ping into a tagged-enum sibling) would
// fail at the docs-validator level rather than only at test runtime.
//
// Selector form: the single-line statement
// `assert_eq!(value["kind"].as_str(), Some("daemon_ping"));`
// appears exactly once in main.rs (line 5853). Many sibling
// envelopes assert their own kind literal with the same macro
// signature but a different `Some(...)` argument; the
// `"daemon_ping"` literal in the selector and the brace-scoping to
// ping_json_pins_top_level_schema together isolate this match from
// bootstrap_result, peer_list, peer_revoke, intents_resume,
...
// range). The validator pins the single line. Same single-line
// pattern as the sibling
// validate-bootstrap-result-kind-literal-value-pin-line-refs.mjs.
...
const selector = 'assert_eq!(value["kind"].as_str(), Some("daemon_ping"));';

const docsRegex =
  /- `kind`: literal string `"daemon_ping"`\. Pinned at the value level by `main\.rs:(\d+)` \(asserts `value\["kind"\]\.as_str\(\) == Some\("daemon_ping"\)`\), so a future kind-rename fails the test rather than silently rewriting the discriminator string\./;
const docsLabel = "daemon_ping kind literal value pin citation";
const docsTemplate =
  '- `kind`: literal string `"daemon_ping"`. Pinned at the value level by `main.rs:N` (asserts `value["kind"].as_str() == Some("daemon_ping")`), so a future kind-rename fails the test rather than silently rewriting the discriminator string.';
...
if (errors.length > 0) {
  console.error(
    "validate-daemon-ping-kind-literal-value-pin-line-refs: failed",
  );
...
console.log(
  `validate-daemon-ping-kind-literal-value-pin-line-refs: ok (kind-value main.rs:${selectorLine})`,
);
