...
// intent_result status type-level pin line-ref drift guard.
// docs/ipc-and-http-gateway.md line 236 cites the main.rs line range
// for the type-level pin on `value["status"].is_string()` inside
// `intent_result_json_pins_top_level_schema`. The assertion is the
...
// validate-audit-recent-type-level-pin-line-refs.mjs convention rather
// than the single-line cite used by the sibling `text` field one bullet
// below (validate-intent-result-text-type-level-pin-line-refs.mjs).
//
// Selector form: `value["status"].is_string(),` recurs across many
// pins-tests (intents_resume_ok, intents_resume_error, bootstrap_result,
// audit_verify, ...). Brace-scoping to intent_result_json_pins_top_
// level_schema isolates this match.
//
// docsRegex anchoring: line 615 carries a sibling `status` bullet for
// the intents_resume_ok envelope using an em-dash separator and the
// "daemon-returned outcome status (typically" phrasing. Line 236's
// intent_result bullet uses a colon separator and the "the outcome
// status (e.g.," phrasing plus the canonical "Pinned as a string by"
// cite, so the regex anchors on the colon-and-"outcome status (e.g.,"
// prefix, captures the range, and requires the value-enumeration trailer.
...
const docsRegex =
  /- `status` \(string\): the outcome status \(e\.g\., `"ok"`\)\. Pinned as a string by `main\.rs:(\d+)-(\d+)` — never an object or array\. Specific value enumeration lives with the daemon's intent dispatcher rather than this docs surface\./;
const docsLabel = "intent_result.status type-level pin citation";
const docsTemplate =
  "- `status` (string): the outcome status (e.g., `\"ok\"`). Pinned as a string by `main.rs:N-M` — never an object or array. Specific value enumeration lives with the daemon's intent dispatcher rather than this docs surface.";
...
if (errors.length > 0) {
  console.error(
    "validate-intent-result-status-type-level-pin-line-refs: failed",
  );
...
console.log(
  `validate-intent-result-status-type-level-pin-line-refs: ok (status main.rs:${startLine}-${endLine})`,
);
