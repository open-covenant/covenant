...
// memory_backfill schema literal value pin line-ref drift guard.
// docs/ipc-and-http-gateway.md line 649 cites a main.rs range for the
// value-level pin on
// `value["schema"].as_str() == Some("covenant.memory.backfill.v1")`
// inside `memory_backfill_json_pins_top_level_schema`. The sibling
// is_string type pin at main.rs:5628-5631 is already cited; this
...
//
// Brace-scoping to memory_backfill_json_pins_top_level_schema
// isolates 5633 from 5570. Both selectors are first lines of
...
// validate-settlement-backfill-schema-value-pin-line-refs.mjs.
...
const docsRegex =
  /The literal value `"covenant\.memory\.backfill\.v1"` is also pinned at the value level by `main\.rs:(\d+)-(\d+)` \(asserts `value\["schema"\]\.as_str\(\) == Some\("covenant\.memory\.backfill\.v1"\)`\), so a future `\.v2` bump fails the test rather than silently rewriting the schema string\./;
const docsLabel = "memory_backfill schema value pin citation";
const docsTemplate =
  "The literal value `\"covenant.memory.backfill.v1\"` is also pinned at the value level by `main.rs:N-M` (asserts `value[\"schema\"].as_str() == Some(\"covenant.memory.backfill.v1\")`), so a future `.v2` bump fails the test rather than silently rewriting the schema string.";
...
if (errors.length > 0) {
  console.error(
    "validate-memory-backfill-schema-value-pin-line-refs: failed",
  );
...
console.log(
  `validate-memory-backfill-schema-value-pin-line-refs: ok (schema-value main.rs:${startLine}-${endLine})`,
);
