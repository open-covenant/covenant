...
// peer_revoke envelope type-level pin line-ref drift guard.
// docs/ipc-and-http-gateway.md line 335 cites a single main.rs range
// for the peer_revoke.outcome (tagged-enum object) type pin.
//
// The cite is currently correct under the assert!-opener-to-closer
// 4-line convention. This validator lands proactively — sibling
// envelope pins-tests saw ~222-line drift events before their
// type-level pin cites were guarded; peer_revoke is added to the
// guarded set before its first drift event lands.
//
// The validator scopes its lookup to the brace-balanced
// `peer_revoke_json_pins_top_level_schema` fn body. Three
// value["outcome"].is_object() selectors live in main.rs (line 5166 in
// peer_revoke pins, plus two more in sibling pins-tests); brace-scoping
// plus exact-trim-match isolates the peer_revoke one.
...
const docsRegex =
  /The top-level object has exactly two keys \(`kind` and `outcome`\); the inner `outcome` is pinned by the schema test at `main\.rs:(\d+)-(\d+)` to be a JSON object, never a string blob\./;
const docsLabel = "peer_revoke.outcome type-level pin citation";
const docsTemplate =
  "the inner `outcome` is pinned by the schema test at `main.rs:N-M` to be a JSON object, never a string blob.";
...
if (errors.length > 0) {
  console.error("validate-peer-revoke-type-level-pin-line-refs: failed");
...
console.log(
  `validate-peer-revoke-type-level-pin-line-refs: ok (peer_revoke.outcome main.rs:${startLine}-${endLine})`,
);
