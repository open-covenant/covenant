...
// ping envelope type-level pin line-ref drift guard.
// docs/ipc-and-http-gateway.md line 226 cites a single main.rs range
// for the ping.status (string) type pin inside the
// ping_json_pins_top_level_schema fn body.
//
// Before this validator landed, the docs prose said the field was a
// literal `"ok"` string but carried no main.rs cite; the assertion at
// main.rs:5854-5857 (assert!(value["status"].is_string(), ...);) was
...
// a non-string type surfaces at the docs-validator level (not just
// at test runtime).
//
// Sibling collision risk: the `value["status"].is_string(),` selector
// appears in two other pin tests in main.rs:
//
//   - intents_resume_error_json_pins_top_level_schema at main.rs:5381
//     (intent_result_error.status type pin).
//   - intent_result_json_pins_top_level_schema at main.rs:5790
//     (intent_result.status type pin).
//
// Both risks are addressed by scoping the selector lookup to the
// brace-balanced `ping_json_pins_top_level_schema` fn body — the
// sibling selectors live in different test fns and cannot contaminate
// the result. The docsRegex anchors on the ping-specific phrase
// "the daemon only returns this envelope when it has accepted the
// request and produced a `Response::Pong`", which neither the
// intents_resume_error nor intent_result status bullets contain.
//
...
const docsRegex =
  /- `status`: literal string `"ok"` — the daemon only returns this envelope when it has accepted the request and produced a `Response::Pong`; failures surface as a non-zero CLI exit rather than a non-`"ok"` payload, so consumers can branch on transport success alone\. Pinned as a string by `main\.rs:(\d+)-(\d+)` — never an integer or boolean\./;
const docsLabel = "ping.status type-level pin citation";
const docsTemplate =
  "Pinned as a string by `main.rs:N-M` — never an integer or boolean.";
...
if (source) {
  ...
  if (testMatches.length !== 1) {
    fail(
      `${sourcePath}: expected exactly 1 "fn ${testFnName}" inside tests module but found ${testMatches.length}; remediation: confirm the ping pins-test still exists and is not renamed or duplicated`,
    );
  } else {
...
if (errors.length > 0) {
  console.error("validate-ping-type-level-pin-line-refs: failed");
...
console.log(
  `validate-ping-type-level-pin-line-refs: ok (ping.status main.rs:${startLine}-${endLine})`,
);
