...
// docs/ipc-and-http-gateway.md line 188 cites a single main.rs range
// for the receipt_batch_list.limit (u64) type pin inside the
// receipt_batch_list_json_pins_top_level_schema fn body.
//
// Before this validator landed, the docs prose described `limit` as
// "(u64)" with no main.rs cite. The assertion at main.rs:7092-7095
...
// `receipt_batch_list_json_pins_top_level_schema` fn body. The
// value["limit"].is_u64(), selector is shared with sibling envelopes
// (e.g., receipt_batch_flushed) inside the same source file; the
// brace-scoping plus exact-trim-match isolates the receipt_batch_list
// one. The range is derived as assert!-opener-to-closer (4-line
...
const docsLabel = "receipt_batch_list.limit type-level pin citation";
...
console.log(
  `validate-receipt-batch-list-type-level-pin-line-refs: ok (receipt_batch_list.limit main.rs:${startLine}-${endLine})`,
);
