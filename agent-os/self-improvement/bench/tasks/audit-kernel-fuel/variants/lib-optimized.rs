...
mod imp {
    ...
    fn event_hash(line: &[u8]) -> [u8; 64] {
        hex64(Sha256::digest(line).as_ref())
    }

    fn link(prev_hex: &[u8; 64], event_hex: &[u8; 64]) -> [u8; 64] {
        let mut h = Sha256::new();
        h.update(prev_hex);
        h.update(b"\n");
        h.update(event_hex);
        hex64(h.finalize().as_ref())
    }
    ...
    fn hex_str(buf: &[u8; 64]) -> String {
        // buf is always ascii hex written by hex64
        String::from_utf8(buf.to_vec()).expect("ascii hex")
    }
    ...
    pub fn verify_chain(events_jsonl: &[u8], anchors_jsonl: &[u8]) -> ChainReport {
        ...
        let mut previous: [u8; 64] = [b'0'; 64];
        for (index, line) in event_lines.iter().enumerate() {
            let event_hex = event_hash(line);
            let chain_hex = link(&previous, &event_hex);
            match serde_json::from_slice::<EventFields>(line) {
                Ok(event) => {
                    let matched = match anchors.get(index) {
                        Some(Some(actual)) => {
                            if actual.index == index as u64
                                ...
                                && actual.chain_hash_hex.as_bytes() == &chain_hex[..]
                            {
                                None
                            } else {
                                Some(Failure::EntryMismatch { index: index as u64 })
                            }
                        }
                        Some(None) => Some(Failure::EntryMismatch { index: index as u64 }),
                        None => Some(Failure::EntryMissing { index: index as u64 }),
                    };
        ...
        ChainReport {
            ...
            root_hash_hex: hex_str(&previous),
            failures,
        }
    }
    ...
    pub fn fold_chain(lines: &[&[u8]]) -> Vec<ChainEntry> {
        let mut previous: [u8; 64] = [b'0'; 64];
        debug_assert_eq!(&previous[..], ZERO_CHAIN_HASH.as_bytes());
        ...
        for (index, line) in lines.iter().enumerate() {
            ...
            entries.push(ChainEntry {
                ...
                event_id,
                timestamp_ms,
                event_hash_hex: hex_str(&event_hex),
                previous_hash_hex: hex_str(&previous),
                chain_hash_hex: hex_str(&chain_hex),
            });
            previous = chain_hex;
        }
        entries
