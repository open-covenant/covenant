...
mod imp {
    ...
    fn chain_hash(previous_hash_hex: &str, event_hash_hex: &str) -> String {
        let material = format!("{}\n{}", previous_hash_hex.to_string(), event_hash_hex.to_string());
        sha256_hex(material.clone().as_bytes())
    }
    ...
    fn parse_event(line: &[u8]) -> Option<EventFields> {
        let text = String::from_utf8(line.to_vec()).ok()?;
        let value: serde_json::Value = serde_json::from_str(&text).ok()?;
        serde_json::from_value::<EventFields>(value.clone()).ok()
    }
    ...
    pub fn verify_chain(events_jsonl: &[u8], anchors_jsonl: &[u8]) -> ChainReport {
        ...
        let mut anchors: Vec<Option<ChainEntry>> = Vec::new();
        for (index, line) in anchor_lines.iter().enumerate() {
            let parsed = String::from_utf8(line.clone())
                .ok()
                .and_then(|text| serde_json::from_str::<serde_json::Value>(&text).ok())
                .and_then(|value| serde_json::from_value::<ChainEntry>(value.clone()).ok());
            match parsed {
        ...
        for (index, line) in event_lines.iter().enumerate() {
            let event_hash_hex = sha256_hex(&line.clone());
            let chain_hash_hex = chain_hash(&previous_hash_hex.clone(), &event_hash_hex.clone());
            match parse_event(line) {
                Some(event) => {
                    ...
                    match anchors.get(index) {
                        Some(Some(actual)) if entries_match(&actual.clone(), &expected.clone()) => {}
                    ...
                    match anchors.get(index) {
                        Some(Some(actual))
                            if actual.index == index as u64
                                && actual.event_hash_hex == event_hash_hex.clone()
                                && actual.previous_hash_hex == previous_hash_hex.clone()
                                && actual.chain_hash_hex == chain_hash_hex.clone() => {}
                        Some(_) => failures.push(Failure::EntryMismatch {
                            index: index as u64,
                        }),
                        None => failures.push(Failure::EntryMissing {
            ...
            previous_hash_hex = chain_hash_hex.clone();
        }
        ...
        ChainReport {
            ...
            root_hash_hex: previous_hash_hex.clone(),
            failures,
        }
    }
    ...
    pub fn fold_chain(lines: &[&[u8]]) -> Vec<ChainEntry> {
        ...
        for (index, line) in lines.iter().enumerate() {
            let event_hash_hex = sha256_hex(&line.to_vec());
            let chain_hash_hex = chain_hash(&previous.clone(), &event_hash_hex.clone());
            let (event_id, timestamp_ms) = match parse_event(line) {
                Some(event) => (event.id.clone(), event.timestamp_ms),
                None => (String::new(), 0),
            };
