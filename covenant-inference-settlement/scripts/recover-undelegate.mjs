...
import {
  PROGRAM, ER_URL, l1, creditsPda, readCredits, foldReceipts, send, ixUndelegateCredits,
} from "../settlement.mjs";
...
const hashes = fs.readFileSync(process.env.HASHES_FILE, "utf8").split("\n").map((s) => s.trim()).filter(Boolean);
...
if (out) {
  ...
  fs.writeFileSync(out, JSON.stringify({ ...prev, undelegateSig: undelSig, finalRoot, finalBalance: finalBal.toString(),
    offchainRefold: refold, rootMatch: finalRoot === refold, balanceMatch: finalBal === EXPECT_BAL, leftHome: !finalState.delegated }, null, 2));
}
if (finalRoot !== refold || finalBal !== EXPECT_BAL || finalState.delegated) process.exit(3);
