...
Top-level keys are pinned by the test at `agent-os/crates/covenant/src/main.rs:5606` (`memory_backfill_json_pins_top_level_schema`), exercised against a dry-run shape, a mutation shape, and an empty-rows dry-run shape; the test also asserts the literal `"covenant.memory.backfill.v1"` schema string and the always-non-null, always-non-empty `savepoint_name` contract documented above. The kind-subfamily sibling `bootstrap_result` is the only remaining envelope in this docs section without a forcing-function test.
