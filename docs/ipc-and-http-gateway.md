...
- **Versioned `covenant.<area>.<verb>.v<n>` schema subfamily.** The newer shape — these envelopes carry a top-level `schema` string (e.g., `"covenant.settlement.backfill.v1"`, `"covenant.memory.backfill.v1"`) with a `.v<n>` version slot, and they do **not** carry a `kind` field. A future `.v2` envelope is a separate shape, not a field rename inside the existing `.v1` envelope. Forcing-function-test coverage in this subfamily is partial: `covenant.settlement.backfill.v1` now has the same `*_pins_top_level_schema` invariant as the kind-subfamily envelopes; `covenant.memory.backfill.v1` does not yet, and its block calls out the gap explicitly.
...
**No forcing-function test**: like `bootstrap_result`, this envelope is built inline by `memory_backfill_json` at `main.rs:4578-4585` rather than via a dedicated `*_pins_top_level_schema` test. The shape is anchored only by this documentation and the inline `serde_json::json!` macro; consumers should treat it as a docs-only contract until a forcing-function test lands. (The sibling `settlement.backfill.v1` envelope above now has a forcing-function test at `main.rs:5543`; the memory sibling is queued to follow.)
