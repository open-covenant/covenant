...
**No forcing-function test**: like the `bootstrap_result` envelope documented above, this envelope is built inline by `settlement_backfill_json` at `main.rs:4565-4576` rather than via a dedicated `*_pins_top_level_schema` test. The shape is anchored only by this documentation and the inline `serde_json::json!` macro; a refactor that drops or renames a key would not fail a unit test. Consumers who depend on this envelope should treat it as a docs-only contract until a forcing-function test lands.
