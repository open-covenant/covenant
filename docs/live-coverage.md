...
| Memory retention | Covered | CLI memory read JSON, purge JSON, and compaction dry-run/apply including JSON envelope | Backfill or quarantine legacy receipts that predate `memory_record_id` correlation. |
| HTTP gateway | Covered | health, version, bearer auth, tools call, capabilities purge mutation with scoped rejection | Broaden protected mutation coverage across memory repair, peer lifecycle, and A2A recovery endpoints. |
...
| A2A mailbox and restart durability | Covered | duplex, admission gate, CLI compact JSON, CLI repair including stale-lease rejection, CLI status JSON, restart replay | Per-peer repair visibility coverage if delegated repair moves beyond operator-owned tasks. |
...
| Local model and full acceptance path | External service | Ollama and full acceptance tests | Use `probe-ollama.mjs` to preflight required models, then expand opt-in live Ollama coverage. |
