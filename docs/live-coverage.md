...
| HTTP gateway | Covered | health, version, bearer auth, tools call with scoped argument rejection, audit purge mutation, capabilities purge mutation with scoped rejection | Broaden protected HTTP mutation coverage across memory repair, peer lifecycle, and A2A recovery endpoints. |
...
| MCP subprocess transport | Covered | CLI tools list JSON, CLI tools call JSON, stdio initialize/list/call, multi-tool stdio compatibility, JSON content, tool-level error results, split responses, large payloads, transport-closed handling | Third-party fixture once selection and dependency provenance are stable. |
