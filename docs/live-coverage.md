...
| A2A mailbox and restart durability | Covered | duplex, admission gate, CLI compact JSON, CLI repair including stale-lease rejection, CLI status JSON, restart replay, opt-in retry scheduler live coverage | Per-peer repair visibility coverage if delegated repair moves beyond operator-owned tasks. |
