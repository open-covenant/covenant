...
- **Record**: `getAsset 4A2fd…` shows AppData `authority.address` and `data.validator` both equal the validator; `type`/`schema`/`hashAlg` match; `responseHash` is 64 hex.
- **Accountability**: `getAssetsByOwner DKxXr…` includes a record with `subject.asset == 4XtUr…`.
