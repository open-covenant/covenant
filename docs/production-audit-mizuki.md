...
**NO-GO for public paid intake. Production readiness: 68/100.**
...
That is not sufficient to open paid intake. The exact release candidate has passed local and isolated-checkout verification, but its new hosted CI workflow has never run, no production deployment or platform-restore receipt exists, the escrow program has not been independently reviewed or deployed immutably, the live model/sandbox route is unbenchmarked, and neither required public mainnet canary has happened. The stated traction and verified-margin targets are also at zero in the supplied evidence.
...
| Deployment and operations              |  8/15 | Reproducible local release checks exist; hosted CI and production execution receipts do not.         |
...
The final application verifier ran against three fresh local Postgres databases and exited successfully.

| Component                        | Final result                                                              |
| -------------------------------- | ------------------------------------------------------------------------- |
| `@covenant/mizuki`               | 199 tests passed; typecheck and build passed                              |
| `@covenant/mizuki-policy-signer` | 106 tests passed; typecheck and build passed                              |
| `@covenant/mizuki-updater`       | 58 tests passed; typecheck and build passed                               |
| `@covenant/coding-gateway`       | 130 tests passed; build passed                                            |
| `@mizuki/web`                    | 19 tests passed; typecheck and optimized production build passed          |
| Release checks                   | Blueprint invariants, Prettier, scoped production dependency audit passed |

Application total: **512 tests passed**.

Escrow evidence was run separately on the frozen source with local `solana-cargo-build-sbf 4.0.0`, platform-tools v1.53, and rustc 1.89. Six host tests passed; an SBPFv3 artifact was built; 24 artifact-backed tests passed; and clippy passed. The artifact has ELF flags `03000000`, raw SHA-256 `fd2b05ab6f4e60d84a1a6f60ae709a40bbf9744690db378c073ac3f0421973c4`, and Solana executable hash `b9c72d619a3d3fde5ba7b45d5db6e31e2ce1d6aa39e79e5002697f679f7b512a`.

This is local evidence, not release provenance. The workflow pins the official Solana 4.0.0 builder image by digest, but that image and workflow job have not run. The generated Next.js `next-env.d.ts` file is intentionally excluded from the source-format gate; the application source and deployment artifacts remain covered.

A separate packaging check assembled the intended scope in a fresh isolated checkout. `pnpm 10.31.0 install --frozen-lockfile` succeeded across all 20 workspaces, all five application builds passed, and the standalone web output succeeded with no `public/` directory present. The GitHub Action pins resolve to their stated releases, the updater Docker base is digest-pinned, and the exact staging allowlist excludes unrelated repository work. The database restore job now compares normalized full data and sequence dumps across all three restored databases and was proven locally on Postgres 16.14. Hosted CI, the updater container build, and platform deployment remain unexecuted.

No evidence in scope proves a deployed GitHub App, live x402 settlement, live facilitator recovery, E2B egress denial, marketplace tool-call reliability, finalized refund, funded bounty, contributor payout, live Render connectivity, backup/PITR recovery, or updater promotion/rollback.
...
### 1. No hosted release-provenance receipt

This audit assesses the exact scoped release candidate intended for the final handoff commit. Its staging allowlist, clean isolated checkout, frozen install, builds, tests, and local artifact hashes are verified. The resulting immutable revision has not yet produced a hosted application or escrow CI receipt, so there is no independent proof that the repository revision reproduces the local result.

Required before any deployment: inspect and commit the exact allowlisted scope, push the immutable revision, run both CI jobs from that revision, retain the application and escrow logs, and compare the hosted artifact digests with the expected release evidence.

### 2. No production deployment or recovery proof

The Render Blueprint defines five services, three durable Postgres databases, private signer/gateway/updater networking, persistent gateway storage, readiness checks, and fail-closed configuration. The local invariant checker passed. There is no receipt proving the platform accepted the Blueprint, all services booted, private service credentials linked correctly, readiness remained green, or database backups can restore financial rows and reconciliation state.

Required: deploy the exact CI revision, run a 30-minute readiness soak, exercise authenticated service connectivity, restart every stateful process, and restore all three databases into an isolated environment. CI now compares complete normalized data and sequence dumps, but the hosted job has not run and it is not a substitute for a platform backup/PITR drill followed by application reconciliation.
...
6. **Backup evidence is local.** Full data and sequence equality is stronger than a migration-only smoke test, but hosted CI and platform backup/PITR restoration remain unexecuted. Run both, then prove application reconciliation against the restored financial state.
...
1. Commit and review the complete scope; run the pinned CI workflow from that immutable revision.
2. Deploy all services and databases; complete connectivity, restart, restore, RPC disagreement, duplicate request, and signer recovery drills.
3. Complete the live route benchmark and publish cost coverage without claiming verified margin.
4. Complete independent escrow review, adversarial devnet canaries, reproducible verification, and immutable deployment.
5. Reverify the 30 screened candidates and confirm the first canary participants. Do not contact maintainers before the internal canaries pass.
