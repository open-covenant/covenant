...
- Capability grants validate non-empty scopes for known action namespaces before signing; dispatch-time enforcement interprets exact `tool.call.*` argument allowlists, scoped `audit.purge` cutoffs, and memory purge/repair/compaction predicates, then otherwise falls back to action predicates.
...
- Dispatch-time capability scope predicates exist for exact `tool.call.*` argument allowlists, `audit.purge` cutoffs, and memory purge/repair/compaction mutations; memory read/write, peer, A2A, and settlement predicates still need enforcement.
