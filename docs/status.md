...
| IPC and HTTP gateway | Implemented, hardening | `covenant-ipc`, unauthenticated `protocol_info`, HTTP `/version`, v1 protocol fixture replay harness, protected HTTP mutation live coverage, daemon and gateway tests, `docs/protocol-versioning.md` | Add v2 fixtures before the first breaking schema change lands. |
...
| Memory store | Implemented, hardening | `covenant-memory`, SQLite, embedding tests, read-only drift reports with exact receipt correlation, scoped read/write/purge/repair/compaction dispatch checks, daemon/HTTP/CLI repair commands, bounded daemon/HTTP/CLI compaction commands, read-only `memory plan-compaction` JSON planning, live compaction coverage, repair and compaction audit rows | Receipt backfill tooling for legacy uncorrelated rows. |
...
| Budget ledger | Implemented, hardening | `covenant-budget`, persisted budget pause checkpoint records, single-use resume claim tests, daemon budget tests, `covenant intents resume latest --json`, live CLI resume test, `docs/budget-pause-checkpoints.md` | Wire checkpoint save/claim into daemon runtime pause and operator-requested shutdown paths. |
