...
| Signed capabilities | Implemented, hardening | `covenant-permissions`, grant-time scope validation for known namespaces, dispatch-time `tool.call.*` argument allowlists, dispatch-time `audit.purge` cutoff enforcement, dispatch-time memory read/write/purge/repair/compaction scope enforcement, dispatch-time A2A send/recv/respond/repair scope enforcement, capability enforcement tests, `docs/capabilities.md` scope contract | Enforce stable scope predicates for peer and settlement paths. |
