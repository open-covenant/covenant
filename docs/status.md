...
| A2A messaging | Implemented, hardening | `covenant-a2a`, leased queue state, task idempotency metadata, lease-age status filters, restart and CLI repair live tests, daemon/HTTP/CLI repair commands, disabled-by-default retry gate, repair and auto-requeue audit rows | Receiver-side idempotency result cache before any background retry scheduler. |
