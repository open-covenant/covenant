...
| A2A messaging | Implemented, hardening | `covenant-a2a`, leased queue state, lease-age status filters, restart live tests, daemon/HTTP/CLI repair commands, repair audit rows | Live CLI repair coverage and explicit idempotency policy before any automatic retry. |
