...
| Memory store | Implemented, hardening | `covenant-memory`, SQLite, embedding tests, read-only drift reports, scoped read/write/purge/repair/compaction dispatch checks, daemon/HTTP/CLI repair commands, bounded daemon/HTTP/CLI compaction commands with stable JSON envelope, live compaction coverage, repair and compaction audit rows | Automatic compaction schedules and exact record-to-receipt correlation. |
