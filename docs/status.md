...
| A2A messaging | Implemented, hardening | `covenant-a2a`, leased queue state, task idempotency metadata, receiver-side idempotency result cache, lease-age status filters, restart and CLI repair live tests, daemon/HTTP/CLI repair commands, disabled-by-default retry gate, opt-in background retry scheduler, repair and scheduler audit rows | Add an explicit task-kind field and per-peer repair visibility coverage if delegated repair moves beyond operator-owned tasks. |
...
| Distribution and SDK ecosystem | Planned, alpha contract defined | `docs/alpha-release-contract.md` defines a source-built alpha boundary, `docs/releases/README.md` defines evidence bundles, and `agent-os/scripts/alpha-release-bundle.mjs` scaffolds local evidence bundles from read-only release evidence | Package installers, stable SDKs, signed release artifacts, and upgrade policy. |
