...
| Signed capabilities | Implemented, hardening | `covenant-permissions`, grant-time scope validation for known namespaces, dispatch-time `tool.call.*` argument allowlists, capability enforcement tests, `docs/capabilities.md` scope contract | Enforce stable scope predicates for memory and audit mutation families. |
