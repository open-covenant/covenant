...
| Distribution and SDK ecosystem | Planned, alpha contract defined | `docs/alpha-release-contract.md` defines a source-built alpha boundary, `docs/releases/README.md` defines evidence bundles, and `agent-os/scripts/alpha-release-evidence.mjs` records read-only release evidence | Package installers, stable SDKs, signed release artifacts, and upgrade policy. |
