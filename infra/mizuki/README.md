...
7. Deploy signer, gateway, updater, and controller first. The runtime predeploy command no longer requires closed admission: with either control open it asserts the live configuration and full readiness of the incoming image, and fails the deploy if the new image could not serve. Closing admission is still the right first move for incident work and key rotation. Deploy the zero-authority shadow and prove its tokenless closed-state probe, then deploy the sole production runtime and prove the authenticated application dependency probe before cutting over the web app. The deployment probe excludes only updater health to keep the controller path acyclic; operator `/readyz` and paid admission still require updater health. Automatic deploys are disabled intentionally.
...
## Pinned escrow review artifact

This entry documents a pinned Mizuki escrow review artifact for reference. It is a documentation record only and does not itself approve a release.
