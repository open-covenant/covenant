import type { ProviderReceipt } from './types.js';
...
export async function probeUsePod(
  ...
  const nonce = 'mizuki-ready';
  const response = await request(usePodUrl(config, 'chat/completions'), {
    method: 'POST',
    headers: usePodHeaders(config),
    body: JSON.stringify({
      model: config.model,
      messages: [{ role: 'user', content: `Call readiness_probe with nonce ${nonce}.` }],
      tools: [
        {
          type: 'function',
          function: {
            name: 'readiness_probe',
            description: 'Return the supplied readiness nonce.',
            parameters: {
              type: 'object',
  ...
  providerReceipt(response, config.model, config.minimumBalance);
  ...
  if (body.model !== config.model) {
    throw new Error('UsePod readiness returned a different model');
  }
  const call = body.choices?.[0]?.message?.tool_calls?.[0]?.function;
  if (call?.name !== 'readiness_probe' || typeof call.arguments !== 'string') {
    throw new Error('UsePod readiness did not return the required tool call');
  }
  let args: unknown;
  try {
    args = JSON.parse(call.arguments);
  } catch {
    throw new Error('UsePod readiness returned invalid tool arguments');
  }
  if (!isRecord(args) || args.nonce !== nonce) {
    throw new Error('UsePod readiness returned the wrong tool nonce');
  }
}
