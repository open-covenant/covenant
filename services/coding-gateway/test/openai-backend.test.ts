...
describe('OpenaiBackend.run', () => {
  it('drives a tool-call turn then completes, mapping events and accumulating usage', async () => {
    ...
    const secondInput = calls[1]!.params.input as Array<{
      type: string;
      call_id: string;
      output: string;
    }>;
  ...
  it('surfaces a tool error back to the model and marks tool.completed with error', async () => {
    ...
    const { client, calls } = fakeClient([
      {
        events: [
          completed({
            ...
            usage: null,
            output: [
              functionCall('c1', 'edit_file', { path: 'g.txt', old_string: 'x', new_string: 'y' }),
            ],
          }),
        ],
      },
    ...
    expect(emitted.find((e) => e.type === 'tool.completed')).toMatchObject({
      tool: 'edit_file',
      error: true,
    });
  ...
  it('surfaces just the message when a failed response has no error code', async () => {
    ...
    const { client } = fakeClient([
      { events: [failed({ id: 'r', error: { message: 'server error' } })] },
    ]);
    const backend = new OpenaiBackend(client);
    await expect(
      backend.run({
    ...
    const { client, calls } = fakeClient([
      {
        events: [
          completed({
            ...
            usage: null,
            output: [
              functionCall('c1', 'edit_file', { path: 'd.txt', old_string: 'x', new_string: 'y' }),
            ],
          }),
        ],
      },
    ...
    const { client, calls } = fakeClient([
      {
        events: [
          completed({
            id: 'r1',
            usage: null,
            output: [functionCall('c1', 'delete_file', { path: 'z' })],
          }),
        ],
      },
      { events: [completed({ id: 'r2', usage: null, output: [messageItem('recovered')] })] },
    ...
    const { client } = fakeClient([
      {
        events: [
          completed({
            id: 'r',
            usage: null,
            output: [functionCall('c', 'read_file', { path: 'f' })],
          }),
        ],
      },
    ]);
    ...
    const { client, calls } = fakeClient([
      {
        events: [
          completed({
            id: 'r1',
            usage: null,
            output: [functionCall('c1', 'bash', { command: 'ls' })],
          }),
        ],
      },
    ...
    const { client } = fakeClient([
      {
        events: [
          completed({
            id: 'r1',
            usage: null,
            output: [functionCall('c1', 'bash', { command: longCmd })],
          }),
        ],
      },
      { events: [completed({ id: 'r2', usage: null, output: [messageItem('done')] })] },
  ...
  it('applies an edit_file call by splicing old_string for new_string once and writing the result', async () => {
    ...
    expect(emitted.find((e) => e.type === 'file.written')).toMatchObject({
      path: 'a.txt',
      bytes: Buffer.byteLength('xx bar yy'),
    });
    const fedBack = calls[1]!.params.input as Array<{ output: string }>;
