...
import { z } from 'zod';
import { MizukiMcpClient } from './client.js';

// The public Mizuki API. An agent that installs this package should be able to
// quote a real issue immediately, so the default points at the live service
// rather than a loopback address only the operator can reach.
const baseUrl = process.env.MIZUKI_API_URL ?? 'https://mizuki.opencovenant.org/api/mizuki';
