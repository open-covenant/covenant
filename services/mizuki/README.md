...
- `POST /v1/preflights` with `{"github_issue_url":"https://github.com/owner/repo/issues/1"}` requires a signed-in maintainer and an explicitly connected repository, then checks both App installations, maintainer authority, attributable authorization evidence, scope, current repository metadata, and validation commands without creating a quote or accepting payment. Cookie-authenticated callers must first fetch `GET /v1/auth/csrf`, send its token as `x-mizuki-csrf-token`, and send the exact configured `Origin`; bearer tokens with `repositories:read` remain CSRF-free.
...
- `GET /v1/account/jobs?limit=20` lists bounded account-linked job history. Scoped API clients require `account:jobs:read`; their default is 20 and the maximum is 100. Browser sessions retain the 100-record default.
